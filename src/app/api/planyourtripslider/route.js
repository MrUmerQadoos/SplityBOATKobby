...
export async function GET(req) {
  try {
    ...
    const existingRecord = await prisma.planYourTripSlider.findFirst()
...
export async function POST(req) {
  try {
    ...
    if (existingRecord) {
      ...
      await prisma.planYourTripSlider.update({
      ...
      const updatedSlider = await prisma.planYourTripSlider.update({
        where: { id: existingRecord.id },
        data: {
          slider: {
      ...
      const newSlider = await prisma.planYourTripSlider.create({
        data: {
          slider: bannerSlider
        }
      })
