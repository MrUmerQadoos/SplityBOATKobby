...
export async function GET(req) {
  try {
    // Fetch the existing PlacesToVisit slider record
    const existingRecord = await prisma.placesToVisitSlider.findFirst()
...
export async function POST(req) {
  try {
    ...
    if (existingRecord) {
      ...
      await prisma.placesToVisitSlider.update({
      ...
      const updatedSlider = await prisma.placesToVisitSlider.update({
        where: { id: existingRecord.id },
        data: {
          slider: {
      ...
      const newSlider = await prisma.placesToVisitSlider.create({
        data: {
          slider: bannerSlider,
        },
      })
