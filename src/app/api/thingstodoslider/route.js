...
export async function GET(req) {
  try {
    ...
    const existingRecord = await prisma.thingTodoSlider.findFirst()
...
export async function POST(req) {
  try {
    ...
    if (existingRecord) {
      ...
      await prisma.thingTodoSlider.update({
      ...
      const updatedSlider = await prisma.thingTodoSlider.update({
        where: { id: existingRecord.id },
        data: {
          slider: {
      ...
      const newSlider = await prisma.thingTodoSlider.create({
        data: {
          slider: bannerSlider
        }
      })
