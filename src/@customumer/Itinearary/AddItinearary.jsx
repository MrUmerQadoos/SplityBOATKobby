...
import { createItinerary } from '@/action/itinerary' // Import the server action
...
const AddItinearary = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await axios.post('/api/itinerary', finalFormData)
      // const response = await createItinerary(finalFormData)
          ...
          <Grid container spacing={5}>
            ...
            <Grid item xs={12} mt={4} className=' flex justify-end mt-4 '>
              <Button variant='contained' type='submit' disabled={isSubmitting}>
                {isSubmitting ? <CircularProgress size={24} color='inherit' /> : ' Add Itinerary'}
              </Button>
            </Grid>
          </Grid>
        </form>
